...
public class RadarView extends View {
    ...
    private ArrayList<Raindrop> mDrops = new ArrayList<>();
     ...
     * @return
     */
    private static int changeAlpha(int sweepColor, int alpha) {
    ...
    private void removeRaindrop() {
        Iterator<Raindrop> iterator = mDrops.iterator();
    ...
    private void generateRaindrop(int cx, int cy, int radius) {
        //同时最多能存在水滴的个数
        if (mDrops.size() < mRaindropNum) {
            //随机产生一个 20以内的随机数 数字刚好是0就产生一滴水滴
            boolean enable = Math.floor(Math.random()*20) == 0;
            if (enable) {
                int x ;
                int y ;
                int offsetX = (int) (Math.random() * (radius - 20));
                int offsetY = (int) (Math.random()*Math.sqrt((radius - 20) * (radius - 20) - offsetX * offsetX));
                ...
                mDrops.add(new Raindrop(x, y, 0, mRaindropColor));
            }
        }
    }
